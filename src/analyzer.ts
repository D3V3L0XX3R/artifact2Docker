...
export function analyzeFile(filePath: string): AnalysisResult {
  ...
  const externalPackages = importLines.filter(
    (pkg) => !pkg.startsWith(".") && !pkg.startsWith("/") && !pkg.startsWith("@/")
  );
...
function hasTailwindUsage(content: string, packages: string[]): boolean {
  if (packages.includes("tailwindcss")) return true;
