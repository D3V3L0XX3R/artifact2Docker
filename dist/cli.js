...
function run(file, options) {
    ...
    if (analysis.hasTailwind)
        writeCss(outputDir, analysis.hasShadcn);
    console.log(pc.green("done"));
    console.log();
...
function writeNginxConfig(outputDir, port) {
    const config = `server {
    listen ${port};
...
function writeCss(outputDir, hasShadcn) {
    const shadcnVars = `
  --background: 0 0% 100%;
