...
export function generate(analysis, opts) {
...
function writeFile(base, relative, content) {
    const fullPath = path.join(base, relative);
    ensureDir(path.dirname(fullPath));
    writeFileSync(fullPath, content, "utf-8");
}
function copySourceFile(analysis, outputDir) {
...
function genViteIndexHtml(projectName) {
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
function genPackageJson(analysis, name) {
...
function genTailwindConfig(analysis) {
...
function genDockerfile(port, analysis) {
...
function genDockerCompose(name, port) {
    return `services:
  app:
    build: .
    container_name: ${name}
...
function genComponentsJson(analysis) {
    return JSON.stringify({
